...
function initializeFullscreenButtons() {
  ...
  // Clear Budget Grid Filters
  const clearFiltersBtn = document.getElementById('btnClearBudgetFilters');
  if (clearFiltersBtn) {
    clearFiltersBtn.addEventListener('click', () => {
      const budgetGridApi = window.getGridApi ? window.getGridApi() : null;
      if (budgetGridApi) {
        budgetGridApi.setFilterModel(null);
        console.log('✅ All filters cleared');

        // Show toast notification
        if (typeof window.showToastMessage === 'function') {
          window.showToastMessage('Filters Cleared', 'All filters have been cleared successfully', 'success');
        }
      } else {
        console.warn('⚠️ Budget Grid API not available');
      }
    });
  }
