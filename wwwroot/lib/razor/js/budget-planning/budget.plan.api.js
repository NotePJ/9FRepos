...
function populateDropdown(elementOrId, apiUrl, defaultText, optionCallback, showSpinner = false, initOptions = null) {
  ...
  fetch(apiUrl)
    ...
    .then(response => {
      // console.log(`Data received for ${elementId}:`, response);
...
async function fetchCoBU(companyID) {
  try {
    ...
    const placeholderText = isBJC ? 'All COBU' : 'All Format';

    // 🆕 Update the first option text (the "All" option)
    const firstOption = cobuFilter.find('option:first');
    ...
    // 🆕 Update COBU/Format label after fetching data
    if (typeof window.updateCobuLabel === 'function') {
      window.updateCobuLabel(companyID);
    }
  } catch (error) {
...
// Fetch BU Code from API based on companyId and cobu
async function fetchBuCodeFromAPI(companyId, cobu) {
  try {
    console.log(`🔌 [API] Fetching BU Code for companyId=${companyId}, cobu=${cobu}`);

    // Construct API URL
    const apiUrl = `${SELECT_API.businessunits}?companyId=${encodeURIComponent(companyId)}&cobu=${encodeURIComponent(cobu)}`;
    ...
    // Extract BuCode from response
    // Adjust property name based on actual API response structure
    const buCode = data.buCode || data.BuCode || data.code || null;

    if (buCode) {
      console.log(`   ✅ BU Code found: "${buCode}"`);
      return buCode;
    } else {
      console.warn(`   ⚠️ No BuCode in response:`, data);
      return null;
    }
...
async function saveBatchBudgetData(batchData, createdBy = 'DevUser') {
  try {
    console.log('📤 Sending batch data to API:', {
      ...
      createdBy: createdBy,
      endpoint: BUDGET_API.saveBatchEntry
    });
    ...
    // Call API
    const response = await fetch(BUDGET_API.saveBatchEntry, {
      method: 'POST',
      headers: headers,
...
window.fetchEmployeeBenefits = fetchEmployeeBenefits; // BJC BU Code API (NEW)
window.fetchBuCodeFromAPI = fetchBuCodeFromAPI;
