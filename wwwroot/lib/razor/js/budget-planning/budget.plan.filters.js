...
function updateFilterLocations() {
  ...
  fetch(`${BUDGET_API.storeNames}${locationQuery}`)
    ...
    .then(data => {
      ...
      const firstOption = storeNameFilter.find('option:first');
      ...
      // 🆕 Update Company/Store name label after fetching data
      if (typeof window.updateCompstoreLabel === 'function') {
        window.updateCompstoreLabel(companyID);
      }
    })
    .catch(error => {
