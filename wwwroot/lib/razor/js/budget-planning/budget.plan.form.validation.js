...
class BudgetFormValidator {
  ...
  setupValidationRules() {
    this.validationRules = {
      common: {
        ...
        dropdown: ['editNewHC', 'editNewPeriod', 'editNewLEPeriod'], // ✅ Text dropdowns (Yes/No, Month names)
        numeric: ['editYear', 'editNOfMonth', 'editLEnOfMonth'], // ✅ Only numeric fields
        decimal: [
  ...
  validateFieldRealTime(fieldConfig, fieldElement) {
    ...
    const isZeroValid = (fieldConfig.id === 'editLEnOfMonth' || fieldConfig.id === 'editNOfMonth') && value === '0';
    const hasValue = value !== '' && (value !== '0' || isZeroValid);
    ...
    // Additional format validation for dropdown fields
    if (this.isDropdownField(fieldConfig.id)) {
      const isValidFormat = this.validateDropdownField(fieldElement);
      if (!isValidFormat) {
        // console.log(`❌ ${fieldConfig.id}: Invalid dropdown selection`);
  ...
  /**
   * Check if field is dropdown field (text-based dropdown)
   */
  isDropdownField(fieldId) {
    return this.validationRules.common.dropdown &&
           this.validationRules.common.dropdown.includes(fieldId);
  }
  ...
  /**
   * Validate dropdown field (non-empty selection)
   * For text-based dropdowns like New HC (Yes/No), New Period (January/February), etc.
   */
  validateDropdownField(field) {
    ...
    // Validate that:
    // 1. Has a value
    // 2. Not a placeholder option (doesn't start with "Select")
    // 3. Not the first option (usually placeholder at index 0)
    const isValid = value &&
                    !selectedText.toLowerCase().startsWith('select') &&
                    field.selectedIndex > 0;

    this.setFieldHighlightState(
      field,
      isValid ? 'valid' : 'invalid',
      'error',
      isValid ? '' : 'Please select a valid option'
    );
  ...
  validateFieldFormats() {
    ...
    // Check dropdown fields (text-based dropdowns)
    if (this.validationRules.common.dropdown) {
      this.validationRules.common.dropdown.forEach(fieldId => {
        const field = document.getElementById(fieldId);
        if (field && this.isFieldVisible(fieldId)) {
          if (!this.validateDropdownField(field)) {
            errors.push(`${this.getFieldLabel(fieldId)} must have a valid selection`);
          }
        }
      });
    }
  ...
  showValidationErrors(result) {
    ...
    // Show alerts if any
    if (alertContent) {
      const errorContainer = $('#offcanvasEditRow .error-container');
      if (errorContainer.length > 0) {
        errorContainer.html(alertContent);
      } else {
        $('#offcanvasEditRow .offcanvas-body').prepend(`<div class="error-container">${alertContent}</div>`);
      }

      // Scroll to top of offcanvas
      $('#offcanvasEditRow .offcanvas-body').scrollTop(0);
    }
  }
  ...
  async handleFormSubmission(mode) {
    ...
    try {
      ...
      // Submit to API (use dynamic base URL for IIS virtual directory support)
      const apiBase = (window.AppConfig && window.AppConfig.apiBaseUrl) ? window.AppConfig.apiBaseUrl : './api/';
      const result = await this.submitToAPI(apiBase + 'budget/save', formData);
      ...
      if (result.success) {
        ...
        if (mode === 'saveAndNew') {
          ...
          setTimeout(() => {
            $('#offcanvasEditRow').offcanvas('hide');
          }, 1500);
        }
      } else {
  ...
  showSubmissionSuccess() {
    ...
    if (errorContainer.length > 0) {
      ...
      $('#offcanvasEditRow .offcanvas-body').prepend(`<div class="error-container">${successAlert}</div>`);
    }
  ...
  showSubmissionError(message) {
    ...
    if (errorContainer.length > 0) {
      ...
      $('#offcanvasEditRow .offcanvas-body').prepend(`<div class="error-container">${errorAlert}</div>`);
    }
