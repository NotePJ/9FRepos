/**
 * PE Bonus By Cost Center Fullscreen Module
 * Handles fullscreen functionality for PE Bonus By Cost Center Grid
 */

// Store original heights for each grid
const originalHeights = {
  peBonusByCostCenter: '500px'
};
...
function getGridDisplayName(gridId) {
  const gridNames = {
    'budgetPEBonusGrid': 'PE Bonus By Cost Center Grid'
  };
...
function initializeFullscreenButtons() {
  // PE Bonus By Cost Center Grid Fullscreen
  const peBonusByCostCenterBtn = document.getElementById('btnTogglepeBonusByCostCenterFullscreen');
  if (peBonusByCostCenterBtn) {
    peBonusByCostCenterBtn.addEventListener('click', () => {
      const peBonusByCostCenterGridApi = window.getGridApi ? window.getGridApi() : null;
      toggleFullscreen('budgetPEBonusGridContainer', 'btnToggleBudgetPEBonusFullscreen', 'budgetPEBonusGrid', peBonusByCostCenterGridApi, originalHeights.peBonusByCostCenter);
    });
  }
...
function initializeFullscreenListeners() {
  ...
  ['fullscreenchange', 'mozfullscreenchange', 'webkitfullscreenchange', 'msfullscreenchange'].forEach(event => {
    document.addEventListener(event, () => {
      // PE Bonus By Cost Center Grid
      const peBonusByCostCenterGridApi = window.getGridApi ? window.getGridApi() : null;
      handleFullscreenChange('budgetPEBonusGridContainer', 'btnToggleBudgetPEBonusFullscreen', 'budgetPEBonusGrid', peBonusByCostCenterGridApi, originalHeights.peBonusByCostCenter);
    });
  });
  ...
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape') {
      const container = document.getElementById('budgetPEBonusGridContainer');
      if (container && container.classList.contains('fallback-fullscreen')) {
        const gridName = getGridDisplayName('budgetPEBonusGrid');
        exitFallbackFullscreen(container, 'budgetPEBonusGrid', originalHeights.budgetPEBonus);
        const button = document.getElementById('btnToggleBudgetPEBonusFullscreen');
        if (button) {
          button.innerHTML = '⛶';
          button.title = 'Toggle Fullscreen';
        }
