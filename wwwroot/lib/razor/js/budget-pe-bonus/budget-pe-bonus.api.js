...
function fetchCompanies() {
  populateDropdown('companyFilter', BUDGET_PE_BONUS_API.companies, 'Select Company',
    (opt, item) => {
...
async function fetchCoBU(companyId) {
  try {
    ...
    const response = await fetch(`${BUDGET_PE_BONUS_API.cobu}?companyId=${companyId}`);
...
async function fetchBudgetYears(companyId) {
  try {
    ...
    const response = await fetch(`${BUDGET_PE_BONUS_API.budgetYears}?companyId=${companyId}`);
...
async function fetchCostCenters(companyId) {
  try {
    ...
    let apiUrl = `${BUDGET_PE_BONUS_API.costCenters}?companyId=${companyId}`;
...
async function fetchBudgetPEBonusData(filters) {
  try {
    ...
    const response = await fetch(`${BUDGET_PE_BONUS_API.getData}`, {
      method: 'POST',
      headers: {
    ...
    const data = await response.json();
    console.log(`Received ${data.length} cost center records`);
    return data;
  } catch (error) {
    console.error('Error fetching PE Bonus data:', error);
...
window.fetchBudgetPEBonusData = fetchBudgetPEBonusData;
window.fetchBudgetData = fetchBudgetPEBonusData;
console.log('Budget PE Bonus API module loaded');
