/**
 * PE Headcount Filter Validation - Real-time Dropdown Validation with Success Icons
...
/**
 * PE Headcount Filter Validator - Lightweight version for filter validation only
 */
class PEHeadcountFilterValidator {
  constructor() {
  ...
  initialize() {
    ...
    try {
      ...
      console.error('❌ Error initializing PE Headcount Filter Validator:', error);
    }
  }
    ...
    setTimeout(() => {
      this.requiredFilters.forEach(filterId => {
        ...
        if (field) {
          ...
          $(field).off('.peHeadcountValidation');
          ...
          if (isSelect2) {
            ...
            $(field).on('select2:select.peHeadcountValidation select2:unselect.peHeadcountValidation select2:clear.peHeadcountValidation change.peHeadcountValidation', (e) => {
              setTimeout(() => {
            ...
            $(field).on('change.peHeadcountValidation', (e) => {
      ...
      const optionalFilters = ['cobuFilter', 'costcenterFilter'];
  ...
  forceSetupValidation() {
    console.log('🔧 Force setup PE Bonus filter validation...');
    this.setupFilterValidation();
  }
  ...
  destroy() {
    ...
    this.isInitialized = false;
    console.log('🗑️ PE Headcount Filter Validator destroyed');
  }
}

// Create and initialize global instance
window.peHeadcountFilterValidator = new PEHeadcountFilterValidator();
...
$(document).ready(function() {
  setTimeout(() => {
    if (window.peHeadcountFilterValidator && !window.peHeadcountFilterValidator.isInitialized) {
      ...
      console.log('🔧 PE Headcount Filter Validator auto-initialized');
    }
...
$(document).on('select2:ready', function(e) {
  if (window.peHeadcountFilterValidator && window.peHeadcountFilterValidator.isInitialized) {
    window.peHeadcountFilterValidator.forceSetupValidation();
  }
});

// Global helper function for debugging
window.testPEHeadcountValidation = function() {
  console.log('🧪 Testing PE Headcount Filter Validation System...');

  if (window.peHeadcountFilterValidator) {
    console.log('📋 Required filters:', window.peHeadcountFilterValidator.requiredFilters);
    const result = window.peHeadcountFilterValidator.validateAllFilters();
    console.log('Validation result:', result);
  } else {
    console.log('❌ PE Headcount filter validator not found');
  }
};

console.log('✅ PE Headcount Filter Validator loaded');
console.log('   ⚡ Real-time validation for Company & Budget Year dropdowns');
console.log('   ✅ Green checkmark success indicators');
console.log('   🔧 Debug: testPEHeadcountValidation()');
